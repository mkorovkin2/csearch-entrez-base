...
class ChamberSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # file-output.py
        f = open('json_respnonse_scripty.txt', 'w+')
        ...
        id_string = ""
        #id_list_int = list()
        id_list_strings = link_refs["esearchresult"]["idlist"]
        #for id in link_set:
        ...
        for uid in result_set:
            ...
            if uid != "uids":
                if "authors" in result_set[str(uid)]:
                    ...
                    df_title = result_set[str(uid)]["title"]
                        ...
                        if above_element == "authors":
                            for author_element in result_set[str(uid)]["authors"]:
                                ...
                                if response.meta["layer"] == 0:
                                    add_data(df_uid, df_author, response.meta["layer"], df_date, df_uid, response.meta["fb"], df_title)
                                else:
                                    add_data(df_uid, df_author, response.meta["layer"], df_date, response.meta["parent"], response.meta["fb"], df_title)
...
def add_data(id, author, layer, date, parent, f_b, title):
    author = author.lower()

    # Create a temporary dataframe to hold the function's inputs
    temp_df = pd.DataFrame(data=[date, id, author, layer, parent, f_b, title]).transpose()
...
url = ""
if filter:
    url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=(" + format_search(search_name) + "%5BAuthor%5D)%20AND%20(\"2010\"%5BDate%20-%20Publication%5D%20%3A%20\"3000\"%5BDate%20-%20Publication%5D)&retmax=" + str(retmax) + "&retmode=json"
else:
    url2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=" + format_search(search_name) + "[author]&retmax=" + str(retmax) + "&retmode=json"
    url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=" + search_name + "[author]&retmax=100&retmode=json"
ChamberSpider.start_urls = [ url ]
process.crawl(ChamberSpider)
x = process.start()

# Output the contents of the dataframe into a csv file
file_name = to_id + str(filter) #str(search_name) + "_data_ms" + str(datetime.datetime.now().microsecond)
