...
def run(search_name, output_name):
    retmax = 50
    filter = True

    # Name of the author to investigate
    #search_name = "Alexandre de Brevern"
    formatted_search_name = format_author(search_name)
    ...
    url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=" + formatted_search_name + "[author]&retmax=" + str(retmax) + "&retmode=json"
    ChamberSpider.start_urls = [ url ]
    process.crawl(ChamberSpider)
    x = process.start()

    # Output the contents of the dataframe into a csv file
    file_name = str(search_name) + "_data_ms" + str(datetime.datetime.now().microsecond)
    gdf.to_csv(path_or_buf="/Users/mkorovkin/Desktop/author_csv/citations_" + output_name + ".csv", encoding='utf-8')
    print(file_name + ".csv")
